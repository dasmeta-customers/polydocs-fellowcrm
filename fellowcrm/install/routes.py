...
def sys_info():
    ...
    if v and int(v[0]) < 3 and int(v[1]) < 5:
        return render_template("install/error.html", title="Fellow CRM installation failed",
                               reason=f"Python version >= {current_app.config['PYTHON_VER_MIN_REQUIRED']} is required for fellowcrm")
    env_vars = {
